...
def create_response_message(received_message, username):
    ...
    head = f'HTTP/1.1 200 OK{salto}Server: nginx/1.17.0{salto}Date: {get_current_datetime()}{salto}Content-Type: text/html; charset=utf-8{salto}Content-Length: {len(body.encode())}{salto}Connection: keep-alive{salto}Access-Control-Allow-Origin: *{salto}{custom_header}{doble_salto}'
